"""Radio settings serialization, validation, and application."""
...
def _settings_unavailable_payload(message: str, requires_image: Any=False, error_text: Any="") -> Any:
...
def _setting_path(parts: Any) -> Any:
...
def _serialize_setting_node(node: Any, path_parts: Any) -> Any:
...
def _serialize_radio_settings(settings_tree: Any) -> Any:
...
def _validate_and_apply_radio_settings(radio: Any, serialized_groups: Any, apply_changes: Any=False) -> Any:
...
def get_radio_settings(module_name: str, class_name: str) -> Any:
    ...
    if issubclass(radio_cls, chirp_common.CloneModeRadio) and not runtime_images._has_cached_image(
    ...
    radio = runtime_images._best_effort_radio_instance(module_name, class_name)
    rf = radio.get_features()
    if not bool(getattr(rf, "has_settings", False)):
        return _settings_unavailable_payload(
...
def validate_radio_settings(module_name: str, class_name: str, settings_groups: Any) -> Any:
    ...
    radio = runtime_images._best_effort_radio_instance(module_name, class_name, require_cached=False)
    try:
