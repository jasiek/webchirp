...
import { createBulkEditChannels } from "./ui/bulk-edit-channels.js";
...
export function createUiController() {
  ...
  const actions = {
    ...
    isAnyModalOpen: () =>
      ...
      || ctx.bulkEditChannels.isModalOpen()
      || ctx.channelExtra.isModalOpen(),
    openChannelExtra: (rowIdx, trigger) => ctx.channelExtra.openForRow(rowIdx, trigger),
    openBulkEdit: (rows, trigger) => ctx.bulkEditChannels.openForRows(rows, trigger),
  ...
  const bulkEditChannels = createBulkEditChannels(ctx);
  ...
  Object.assign(ctx, {
    settings, table, channelExtra, bulkEditChannels, catalog, repeaterQuery, repeaterMap, codeplugIo, serial,
  });
  ...
  function bindEvents() {
    ...
    // bulk-edit and channel extras editors, then the repeater modals.
    document.addEventListener("keydown", (event) => {
      if (event.key === "Escape") {
        ...
        if (bulkEditChannels.isModalOpen()) {
          bulkEditChannels.closeModal();
          return;
        }
